...
use crate::{
    ...
    components::{Coords, EditorCtx, RegisterData},
    systems::{
        commons::{coords_to_char_idx, display_line, next_col_or_display_width},
        input::dispatch_cmd,
    },
};
...
fn basic_charwise_select(ctx: &mut EditorCtx, span: (Coords, Coords), inclusive: bool) -> String {
    ...
    if inclusive {
        end.col = next_col_or_display_width(&ctx.config, buffer.rope(), buf_view, end);
    }
