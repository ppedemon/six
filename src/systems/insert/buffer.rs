...
pub fn insert_char(config: &Config, buf_view: &mut BufferView, rope: &mut Rope, c: char) -> Damage {
    let char_idx = cursor_to_char_idx(config, buf_view, rope);
    rope.insert_char(char_idx, c);
    patch_curr_line(config, rope, buf_view);
    move_right::<InsertNav>(config, rope, buf_view, 1);
    Damage::Line(buf_view.cursor.row)
}

pub fn enter(config: &Config, buf_view: &mut BufferView, rope: &mut Rope) -> Damage {
    let char_idx = cursor_to_char_idx(config, buf_view, rope);
    ...
    move_down::<InsertNav>(config, rope, buf_view, 1);

    Damage::From(buf_view.cursor.row)
}

pub fn backspace(config: &Config, buf_view: &mut BufferView, rope: &mut Rope) -> Damage {
    if buf_view.cursor.col > 0 {
        ...
        let prev_idx =
            coords_to_char_idx(config, rope, buf_view, Coords::new(cursor.row, prev_col));

        rope.remove(prev_idx..char_idx);
        patch_curr_line(config, rope, buf_view);
...
pub fn delete(config: &Config, buf_view: &mut BufferView, rope: &mut Rope) -> Damage {
    let cursor = buf_view.cursor;
...
fn join_above(config: &Config, buf_view: &mut BufferView, rope: &mut Rope) -> Damage {
    ...
    if prev_idx > 0 && rope.char(prev_idx) == '\n' && rope.char(prev_idx - 1) == '\r' {
        prev_idx -= 1;
    }

    let line = buf_view.display_buf.ensure_line(config, rope, row_above);
    buf_view.cursor.col = line.display_width;
    buf_view.target_col = buf_view.cursor.col;

    rope.remove(prev_idx..char_idx);
    buf_view.display_buf.destroy_from(row_above);
    move_up::<InsertNav>(config, rope, buf_view, 1);
