use crate::{
    ...
    cmd::EditOp,
    components::{BufferId, Coords, EditorCtx, ExState, Focus, MutBuffer},
    systems::{
...
fn handle_session_edit(ctx: &mut EditorCtx, op: EditOp) -> DamageEvent {
    ...
    let damage = match op {
        EditOp::InsertChar(c) => insert_char(&ctx.config, buf_view, &mut buffer.edit(), c),
        EditOp::Tab => insert_char(&ctx.config, buf_view, &mut buffer.edit(), '\t'),
        EditOp::Enter => enter(&ctx.config, buf_view, &mut buffer.edit()),
        EditOp::Backspace => backspace(&ctx.config, buf_view, &mut buffer.edit()),
        EditOp::Delete => delete(&ctx.config, buf_view, &mut buffer.edit()),
    };
...
pub fn clear_ex(ctx: &mut EditorCtx) {
    ...
    ex_session.edit().remove(0..len_chars);
...
fn handle_ex_edit(ctx: &mut EditorCtx, op: EditOp) {
    ...
    match op {
        EditOp::InsertChar(c) => insert_char(&ctx.config, buf_view, &mut ex_session.edit(), c),
        EditOp::Tab => insert_char(&ctx.config, buf_view, &mut ex_session.edit(), '\t'),
        EditOp::Backspace => backspace(&ctx.config, buf_view, &mut ex_session.edit()),
        EditOp::Delete => delete(&ctx.config, buf_view, &mut ex_session.edit()),
